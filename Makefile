...
# NOTE: for some devices /dev/xxx is not created (e.g., usb keyboard) - for such devices find another way to automatically determine devnum and busnum ANSWER: use SUBSYSTEM=="usb" ... SYMLINK+=... - then /dev/unique_name will be created
boot: all
	@! ps -fC qemu-system-x86_64 | grep -v newcd-qemu | grep -q qemu || ( echo ALREADY RUNNING; false )
