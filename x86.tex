...
{\bf reimplement everything as in tel.w - see https://www.kernel.org/doc/Documentation/input/input.txt}

NOTE: any+any may be used on numpad (for example, ``enter'' key may be used as a modifier);
NumLock key cannot be used as a modifier.

\N
Build virtual machine (this step is only necessary if "make boot" will not work, because
pre-compiled image is automatically installed with the OS):
\begtt
./build-x86.sh
\endtt
\medskip
...
\N
For next step to work, add this to "/etc/udev/rules.d/99-local-udev.rules" (done automatically in installer):
\begtt
SUBSYSTEMS=="usb", ATTRS{idVendor}=="<vendorid>", ATTRS{idProduct}=="<productid>", GROUP:="dialout"
\endtt
\medskip

\N
Plug in usb keyboard or numpad (in addition to your main keyboard), see dmesg for {\sl vendorid\/} and {\sl productid\/}
values, set these values in Makefile and start qemu:
\begtt
make boot
\endtt
\medskip
...
"while true; do ps -ww|grep [s]leep; done"), script from `case 2' must behave
